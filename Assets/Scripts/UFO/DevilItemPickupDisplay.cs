...
/// <summary>
/// Devilキャッチャーで獲得したアイテムを、画面左下などで3D回転表示するポップアップUI。
...
public class DevilItemPickupDisplay : MonoBehaviour, ILanguage
{
    ...
    public void ShowPickedItem(GameObject pickedObject)
    {
        ...
        {
            Debug.LogWarning($"[DevilItemPickupDisplay] '{itemName}' に対応するItemDataがitemDatabaseに見つからないため、ポップアップ表示をスキップします。");
            return;
        }
    ...
    private void ShowModel(GameObject prefab, string displayName)
    {
        ...
        {
            Debug.LogWarning("[DevilItemPickupDisplay] displayStage がインスペクターに設定されていません。");
            return;
        }
        ...
        if (layer < 0)
        {
            Debug.LogWarning($"[DevilItemPickupDisplay] レイヤー '{displayLayerName}' が存在しません。Tags and Layers で作成してください。");
        }
        else
        {
