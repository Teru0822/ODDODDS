...
public class DevilClawCarrier : MonoBehaviour
{
    [Header("設定")]
    ...
    private void Start()
    {
        ...
        Debug.Log($"[DevilClawCarrier] Start: {gameObject.name} initialized. BoxCollider Size: {_boxCollider.size}, Parent: {transform.parent?.name}");
    ...
    private void FixedUpdate()
    {
        ...
        {
            Debug.LogWarning($"[DevilClawCarrier] OverlapBox のヒット数がバッファ上限({_overlapBuffer.Length})に達しました。取りこぼしがある可能性があります。", this);
        }
        ...
        {
            Debug.Log($"[DevilClawCarrier] FixedUpdate Tick. Position: {currentPosition.ToString("F3")}, Delta: {delta.ToString("F4")}, RawHitsCount: {hitCount}, BoxSize: {_boxCollider.size}");
        ...
        for (int i = 0; i < hitCount; i++)
        {
            ...
            if (rb != null)
            {
                ...
                if (showDebugLogs)
                {
                    Debug.Log($"[DevilClawCarrier] Carried object: {rb.name} by {appliedDelta.ToString("F4")} (Efficiency: {effectiveEfficiency:F2}) to {newPos.ToString("F3")}");
                }
            }
        }
        ...
        {
            Debug.Log($"[DevilClawCarrier] Total carried this frame: {carriedCount} objects. (Applied Delta: {appliedDelta.ToString("F4")})");
        }
    }
}
