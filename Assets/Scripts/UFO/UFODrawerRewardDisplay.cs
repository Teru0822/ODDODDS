...
/// <summary>
/// UFOキャッチャーの機体にある引き出しの中で、獲得したアイテム（金/銀/銅コイン・黒ダイヤ）を見せる演出。
...
/// 中身をそのまま引き出しごと見せる。
/// </summary>
public class UFODrawerRewardDisplay : MonoBehaviour
{
    ...
    /// <summary>
    /// UFOキャッチャー終了時に呼ぶ。プレイ中に積み上がった中身をそのまま見せるため、引き出しを開くだけ。
    /// </summary>
    ...
    private IEnumerator DrawerRoutine()
    {
        ...
        // 今アクティブなUFOカメラ（frontCamera）自体を、引き出しを見せる位置までLerpで動かす。
        ...
        // （フォーカス表示自体は GameUIManager.HandleUfoModeChanged 側で既に適用済み）
        // ※カウントアップ（DOTween）は独立して動くため、待たずにそのまま引き出しの開閉へ進み、
        //   カウントアップと引き出しアニメーションが同時に進行するようにする
        UFOItemGoal.Instance?.FinalizeSessionRewards();
        ...
        {
            Debug.LogWarning("[UFODrawerRewardDisplay] Drawer Transform が未設定のため、引き出しの開閉演出をスキップしました。");
            yield return new WaitForSeconds(drawerHoldSeconds);
        }
