...
public class RouletteController : MonoBehaviour
{
    ...
    private IEnumerator ExecuteEyeSequenceAndSpinRoutine()
    {
        ...
        // 初期位置・回転の保持
        Vector3 startBasePos = floater != null ? floater.BasePosition : transform.position;
        ...
        // -------------------------------------------------------------------
        // 1a. Y 座標を 4.708 まで上昇
        Vector3 pos1aStart = startBasePos;
        Vector3 pos1aEnd = new Vector3(startBasePos.x, 4.708f, startBasePos.z);
        float t = 0f;
        while (t < durA)
        {
            t += Time.deltaTime;
            ...
            if (floater != null) floater.BasePosition = curPos;
        ...
        while (t < durB)
        {
            ...
            Quaternion curRot = Quaternion.Slerp(startRot, targetWarpRot, e);
            if (floater != null) floater.BasePosition = curPos;
            else transform.position = curPos;
            transform.rotation = curRot;
            yield return null;
        }
        transform.rotation = targetWarpRot;
        ...
        if (floater != null) floater.BasePosition = pos1cEnd;
        else transform.position = pos1cEnd;
        ...
        Vector3 currentBase = floater != null ? floater.BasePosition : transform.position;
        ...
        Vector3 pos4bEnd = new Vector3(startBasePos.x, 4.708f, startBasePos.z);
        t = 0f;
        while (t < durB)
        {
            t += Time.deltaTime;
            ...
            Quaternion curRot = Quaternion.Slerp(targetWarpRot, startRot, e);
            if (floater != null) floater.BasePosition = curPos;
            else transform.position = curPos;
            transform.rotation = curRot;
            yield return null;
        ...
        if (floater != null) floater.BasePosition = startBasePos;
        else transform.position = startBasePos;
