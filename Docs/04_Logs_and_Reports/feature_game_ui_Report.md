...
* **`RoguelikeData.cs` の新規実装**  
  - スキルの属性を表す `SkillType` 列挙型（`None`, `PinBall`, `FallBall`, `DevilCatcher`）を定義。
...
* **`RoguelikeData.json` の新規作成**  
  - ピンボール、フォールボール、デビルキャッチャーに関連する合計7種類のスキルマスタデータを定義。
...
- **スキルアンロック時の実効果の実装**: 
  - 現時点ではスキルがアンロック状態 (`isGet = true`) になるデータ・UIの連携までが構築されているため、今後は各スキル（ピンボールの分裂、フォールボール、デビルキャッチャーの吸引など）が実際にゲーム内で機能を発揮するように、対応するミニゲーム側の制御クラスから `RoguelikeManager.GetUnlockSkillDictionary` のデータを参照・適用するロジックを実装する必要がある。
- **多人数（マルチプレイ）対応**:
