...
* [UFOCameraController.cs](file:///c:/Users/clock/FEVER-CAPITAL/Assets/Scripts/UFOCameraController.cs) を修正しました。
  ...
  * **ライト点灯・消灯仕様の変更**:
    * **UFOキャッチャーの筐体をクリックした瞬間（`OnMachineClicked()`）**に `SetPlaySpotlight(true)` を呼び出し、カメラズーム遷移が開始するタイミングでライトが点灯するように変更（遷移途中から明るくなるように調整）。
...
## 新たに何が出来るようになったか
* UFOキャッチャーの筐体をクリックした瞬間にライトが点灯して中が明るくなるため、カメラがUFOキャッチャーの画面に向かってズームしていく遷移中も常に明るい状態で演出が進行するようになり、プレイ体験が向上しました。
